...
const routes = ["/", "/memory-layout/basic-types/"];
const minimum = 0.95;
    ...
    try {
        for (const route of routes) {
            ...
            for (const [category, score] of Object.entries(scores)) {
                if (score < minimum) failures.push({ route, category, score });
            }
        }
    } finally {
        await chrome.kill();
    }
    assert.deepEqual(failures, [], `Lighthouse scores below ${minimum * 100}: ${JSON.stringify(failures)}`);
